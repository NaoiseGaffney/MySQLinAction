...
then
   ...
   echo "Creating the ${GITPOD_GIT_USER_NAME} user in MySQL"
   sudo mysql -e "CREATE USER '${GITPOD_GIT_USER_NAME}'@'%' IDENTIFIED BY '';"
   echo "Granting privileges"
   sudo mysql -e "GRANT ALL PRIVILEGES ON *.* TO '${GITPOD_GIT_USER_NAME}'@'%' WITH GRANT OPTION;"
fi
